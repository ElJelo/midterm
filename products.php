...
if (isset($_POST['add'])) {
    ...
    $supplierID = $_POST['supplier_id']; // <-- ADD THIS LINE!!
  ...
  // Insert into Stock table (corrected)
$stmt = $pdo->prepare("INSERT INTO Stock (ProductID, QuantityAdded, SupplierID, DateAdded, UserID) VALUES (?, ?, ?, NOW(), ?)");
$stmt->execute([$productID, $stockLevel, $supplierID, $userID]);
...
<div class="container">
    ...
    <div class="form-container">
        ...
        <form method="POST">
          ...
          <?php 
          $suppliers = $pdo->query("SELECT SupplierID, Name FROM Suppliers ORDER BY Name ASC");
          while ($row = $suppliers->fetch()): ?>
            <option value="<?= $row['SupplierID'] ?>"><?= htmlspecialchars($row['Name']) ?></option>
        <?php endwhile; ?>
    </select>
</div>
