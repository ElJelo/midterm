...
if (isAdmin()) {
    ...
    // If staff, only show products and suppliers they manage
    $productsQuery = $pdo->prepare("SELECT * FROM Products WHERE UserID = ? ORDER BY Name");
    $productsQuery->execute([$userID]);
    
    $suppliersQuery = $pdo->prepare("SELECT * FROM Suppliers WHERE UserID = ? ORDER BY Name");
    $suppliersQuery->execute([$userID]);
}
